...
CSX = InitCSX();
resolution = c0/(f_stop*sqrt(substrate.epr))/unit /x(4);
mesh.x=SmoothMeshLines([ 0 MSL.gap/2 ],resolution/2, 1.3 );
mesh.x=SmoothMeshLines([ mesh.x MSL.length+MSL.gap/2 MSL.gap/2+MSL.length+MSL.port_dist],resolution, 1.5 );
mesh.x = SmoothMeshLines( [-mesh.x mesh.x], resolution, 1.4 ,0 );
...
stop  = [-MSL.gap/2,  MSL.width/2, substrate.thickness];
CSX = AddBox(CSX,'gold',500,start,stop);
start = [MSL.gap/2,   -MSL.width/2, substrate.thickness];
